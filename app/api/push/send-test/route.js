...
async function POSTHandler(request) {
  try {
    apiDebugLog('SEND_TEST: Starting...');
    const user = await requireAuthApi(request);
    apiDebugLog('SEND_TEST: User authenticated', { userId: user.id, role: user.role });

    const config = getWebPushConfig();
    apiDebugLog('SEND_TEST: VAPID configured:', config.configured, 'publicKey length:', config.publicKey?.length);
    ...
    apiDebugLog('SEND_TEST: Total subscriptions in DB:', allSubs?.length || 0);
    
    // Log all subscriptions for debugging
    allSubs.forEach((s, i) => {
      apiDebugLog(`SEND_TEST: Sub[${i}]:`, JSON.stringify({
        userId: s.userId,
    ...
    apiDebugLog('SEND_TEST: Looking for subscriptions for userId:', user.id, 'type:', typeof user.id);
    const subs = await findSubscriptionsByUserIds([user.id]);
    apiDebugLog('SEND_TEST: Found subscriptions for user:', subs?.length || 0);
    
    if (subs.length > 0) {
      apiDebugLog('SEND_TEST: First subscription:', JSON.stringify({
    ...
    for (const sub of subs) {
      try {
        apiDebugLog('SEND_TEST: Sending to endpoint', sub.endpoint?.slice(0, 80));
        apiDebugLog('SEND_TEST: Keys present:', !!sub.keys, 'auth:', !!sub.keys?.auth, 'p256dh:', !!sub.keys?.p256dh);
        await sendPushNotification(
          { endpoint: sub.endpoint, keys: sub.keys },
          payload
        );
        results.push({ ok: true, endpoint: sub.endpoint?.slice(0, 30) });
        apiDebugLog('SEND_TEST: SUCCESS');
      } catch (err) {
        ...
        if (statusCode === 403 || statusCode === 410) {
          apiDebugLog('SEND_TEST: Removing invalid subscription:', sub.endpoint?.slice(0, 50));
